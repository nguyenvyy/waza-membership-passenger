import { SEND_REQUEST_WALLET, STOP_REQUEST_WALLET, SAVE_WALLET_INFO, ADD_CASH, TRANSFER_CASH_TO_ELECTRONIC, CLEAR_WALLET, CATCH_ERROR_WALLET } from "./types";
import { getWalletAPI, addMoneyToCashAPI } from "./services";
...
export const transferCashToElectronicRequest = money => async (dispatch, getState) => {
    ...
    try {
        const id = getState().auth.user && getState().auth.user._id
        if(!id) throw new Error(id)
        const status = await transferCashToElectronic(id, money)
        if(status === 400) throw new Error(status)
        addCash(money)
    } catch (error) {
        dispatch(stopRequestWallet())
